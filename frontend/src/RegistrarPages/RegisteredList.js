...
export default class RegisteredList extends Component {
    ...
    ****************************************/
    componentDidMount() {
        ...
        axios.get('/applications/view/all/?registration_status=reg_completed', {
            ...
            if (this._isMounted) {
                ...
                for (var key in res.data) {
                    ...
                    newState.registeredVessels.push(newDict)
                }

                console.log('vessels managed data', newState.registeredVessels)
    ...
    /****************************************
    * renderRegistered() is a dynamic Table generator
    * with information on the vessel
    ****************************************/
    renderRegistered = (registeredVessels, index) => {
        return (
            <tr key={index} onClick={() => this.setState({ currVessels: registeredVessels, showModal: true })}>
                <td>{registeredVessels.ship_name}</td>
                <td>{ConstSwitch.checkPort(registeredVessels.port)}</td>
                <td>{registeredVessels.first_name} {registeredVessels.last_name}</td>
                <td>{DateHelpers.shortDate(registeredVessels.date_approved)}</td>
            </tr>
        )
    }
            ...
            <div>
                ...
                <div className="Registrar">
                    <Breadcrumb>
                        ...
                        <Breadcrumb.Divider />
                        <Breadcrumb.Section active>Registered</Breadcrumb.Section>
                    </Breadcrumb>
                    ...
                    <ReactBootStrap.Table className="Registrar-table" striped bordered hover size="sm">
                        ...
                        <tbody>
                            {this.state.loadingTable === true ?
                                ...
                                :
                                this.state.registeredVessels.length === 0 ?
                                    <tr>
                                        <td colSpan="4"> {"Table is empty. No Registered Vessels."}</td>
                                    </tr>
                                    :
                                    this.state.registeredVessels.map((this.renderRegistered))
                            }
                        </tbody>
